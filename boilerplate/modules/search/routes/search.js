...
const ejsfile = ß.CWD + '/modules/search/public/results.ejs';
const errfile = ß.CWD + '/modules/search/public/err.ejs';
...
function express_search(req, res) {
    ...
    console.log('search: /bin/grep --exclude-dir={.git,log} -nrowE ".{0,100}' + search_term + '.{0,100}"');
...
ß.app.get('/replace/*', function(req, res, next) {
    ...
    ß.lib.search.replace(replace_file, search_term, replace_term, function(err, success) {
        ß.lib.projectfiles.oplog(req.session.username, 'replaced ' + search_term + ' to ' + replace_term + ' in', replace_file);
        express_search(req, res);
    });
