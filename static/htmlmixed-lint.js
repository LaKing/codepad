...
(function(mod) {
    ...
    CodeMirror.registerHelper("lint", "html", function(text, options) {
        ...
        if (!options.indent) // JSHint error.character actually is a column index, this fixes underlining on lines using tabs for indentation
      	...
      	// let us add an additional layer in case we ant to define linter-specific option via options, otherwise take a clone of the defualt options object
        ...
        function processJS(text, options, found) {
            // blocks to be processed will be those with an odd index
            var blocks = text.split(/<script[\s\S]*?>|<\/script>/gi);
            // look at the odd indexed blocks 
            for (var j = 1; j < blocks.length; j += 2) {
