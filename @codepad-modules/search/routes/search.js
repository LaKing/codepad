...
function ref(m) {
    return '<b><span class="cm-atom">' + m + "</span></b>";
}
...
function form_response(data, search_term, replace_term) {
    ...
    for (var i = i_s; i < lines.length; i++) {
        ...
        if (current !== fullpath) {
            ...
            if (replace_term)
                res_send +=
                    ' - <a class="CodeMirror-guttermarker" href="' +
                    abspart +
                    "/replace/" +
                    fullpath +
                    "?&find=" +
                    ...
                    "&replace=" +
                    replace_term +
                    '">REPLACE ALL TO:</a> <span class="term">' +
                    replace_term +
                    "</span>";
...
function express_search(req, res) {
  ...
  */
    ...
    const x = spawn(
        "/bin/grep",
        ["--exclude-dir=.git", "--exclude-dir=log", "--exclude-dir=node_modules", "--exclude-dir=var", "--exclude-dir=cert", "-InRowE", ".{0,100}" + search_term + ".{0,100}"],
        options
    );
